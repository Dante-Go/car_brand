...
if __name__ == '__main__':
    ...
    log=5
    # 产生train.record文件
    image_dir= os.path.join(gbl.dataset_base_path, 'train')
    ...
    # 产生val.record文件
    image_dir=os.path.join(gbl.dataset_base_path, 'val')
    val_labels = os.path.join(gbl.dataset_base_path, 'val.txt')  # 图片路径
#     val_record_output = '/data/car_brand_model/data/record/val{}.tfrecords'.format(resize_height)
    val_record_output = 'val{}.tfrecords'.format(resize_height)
    val_record_output = os.path.join(gbl.dataset_base_path, 'record', val_record_output)
